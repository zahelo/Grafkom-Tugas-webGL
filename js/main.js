...
async function main() {
  ...
  async function loadMaterialsAndTextures(gl, materials, baseHref) {
    ...
    function render(time) {
      ...
      const angle = time * 0.2; // Kecepatan rotasi (atur sesuai kebutuhan)
      const dynamicCameraPosition = [
        Math.sin(angle) * radius, // X berputar
        Math.sin(angle * 0.5) * radius * 0.5, // Y naik-turun (opsional)
        Math.cos(angle) * radius  // Z berputar
      ];
      ...
      let u_world = m4.yRotation(time * 0.1); // Sedikit rotasi objek
      u_world = m4.translate(u_world, ...objOffset);
